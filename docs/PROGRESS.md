...
- **버전:** v1.7.3 배포 중 — 멀티뷰 핵심 기능(4개 플랫폼·레이아웃·녹화·프리셋) 안정화 단계
- **진행:** 저장 중 배너·프리셋 다이얼로그 위치 개선 완료 — main 커밋 2건(85cd6c1·f6d4498), **push(배포) 보류**
- **검증 상태:** verify.sh 초록 + reviewer APPROVE(2회). **브라우저 미확인** — 확인 항목: ① 배너가 리모컨(컨트롤러+설정) 전체 폭 상단 표시·캔버스 불변 ② 접힘+설정 닫힘 시 아이콘+툴팁 ③ 프리셋 다이얼로그가 두 패널 포함 가운데(도킹·분리 양쪽) ④ 분리 팝업에서 배너 표시 시 창 폭 안 튀는지
- **다음 작업:** 브라우저 확인 → 패치 버전 업 → push. 이후 새 작업 선정(큐 잔여 2건은 "이 영역 다시 만질 때" 성격)
- **차단 요소:** 없음
...
- 미해결: **브라우저 미확인** (현재 상태의 확인 항목 4건) — 확인 전 push 금지. 합의된 트레이드오프: 리모컨 분리 중 저장 진행 시 메인 창에는 배너 없음(탭 제목 경고는 유지)
- 다음 작업: 사용자 브라우저 확인 → 패치 버전 업 → push(배포)
