...
import { getBitsColor } from "@/data/color";

function BitsIcon({ amount }) {
  const tier =
    amount >= 10000 ? "red"
    : amount >= 5000 ? "blue"
    : amount >= 1000 ? "green"
  ...
  return (
    <Box
      component="img"
      alt="bits"
      src={`https://static-cdn.jtvnw.net/bits/dark/animated/${tier}/2`}
      sx={{
        width: "1em",
        height: "1em",
        mr: "0.4rem",
...
// Twitch 비트 후원 채팅
function BitsChatRow({ chat }) {
  const { nickname, badges, emojis, message, bitsAmount } = chat;
  const color = getBitsColor(bitsAmount);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mt: "0.2em", display: "flex", alignItems: "center" }}>
        ...
        >
          <BitsIcon amount={bitsAmount} />
          {bitsAmount.toLocaleString("ko-KR")}
        </Box>
      </Box>
    </Box>
  );
...
export default memo(BitsChatRow);
